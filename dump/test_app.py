...
# Fungsi calculate_dynamic_pricing yang dikomentari tetap dipertahankan sesuai permintaan pengguna.
# def calculate_dynamic_pricing(df, group_cols):
#     df['order_date'] = pd.to_datetime(df['order_purchase_timestamp']).dt.date

#     # Hitung jumlah unik order dan seller
#     grouped = df.groupby(group_cols).agg(
#         number_of_orders=('order_id', pd.Series.nunique),
#         number_of_sellers=('seller_id', pd.Series.nunique),
#         historical_cost=('price', 'median')
...
#     # Hitung multiplier berdasarkan demand & supply
#     multiplier = (1 + 0.02 * grouped['number_of_orders']) / (1 + 0.02 * grouped['number_of_sellers'])
#     multiplier = np.clip(multiplier, 0.85, 1.3)  # Batasan antara 85%–130% dari harga awal
...
#         product_id  = st.selectbox("Product Id", options=filtered_products, key='product_id')
#         customer_state = st.selectbox("State", options=list(state_map.keys()))
...
#        st.session_state.final_data = pd.concat([st.session_state.final_data, data_submitted], ignore_index=True)
#        st.success(f"Predicted price for {product_id} in {product_category_enc} for customer who live in {customer_state} are: Rp {price:,.0f}")
